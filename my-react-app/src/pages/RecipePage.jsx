...
function RecipePage() {
  ...
  if (!recipe) {
    return (
      <div className="min-h-screen w-full bg-filipino-cream flex items-center justify-center px-4">
        <div className="text-center">
          <h2 className="text-3xl sm:text-4xl font-bold text-gray-800 mb-4">Recipe Not Found</h2>
          <Link
            to="/"
  ...
  return (
    <div className="min-h-screen w-full bg-filipino-cream">
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden w-full">
          <div className="relative h-64 sm:h-96 w-full overflow-hidden">
            ...
            <div className="absolute bottom-0 left-0 p-4 sm:p-8">
              ...
              <h1 className="text-3xl sm:text-5xl font-bold text-white mb-1 sm:mb-2">{recipe.name}</h1>
              <p className="text-md sm:text-xl text-filipino-light-gold">{recipe.description}</p>
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-4 sm:gap-6 mb-6 sm:mb-8 pb-6 sm:pb-8 border-b border-gray-200">
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                ...
                <div>
                  ...
                  <p className="font-semibold text-gray-800">{recipe.servings} people</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-4 sm:gap-8">
              <div className="md:col-span-1">
                <div className="bg-filipino-cream rounded-xl p-4 sm:p-6 sticky top-20 sm:top-24">
                  <div className="flex items-center gap-2 mb-3 sm:mb-4">
                    ...
                    <h2 className="text-lg sm:text-2xl font-bold text-gray-800">Ingredients</h2>
                  </div>
                  <ul className="space-y-2 sm:space-y-3">
                    {recipe.ingredients.map((ingredient, index) => (
                      <li key={index} className="flex items-start gap-2">
                        <span className="w-2 h-2 bg-filipino-red rounded-full mt-2 flex-shrink-0"></span>
                        <span className="text-sm sm:text-gray-700">{ingredient}</span>
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              <div className="md:col-span-2">
                <div className="mb-6 sm:mb-8">
                  <h2 className="text-lg sm:text-2xl font-bold text-gray-800 mb-4 sm:mb-6">Instructions</h2>
                  <div className="space-y-4 sm:space-y-6">
                    {recipe.instructions.map((instruction, index) => (
                      <div key={index} className="flex gap-3 sm:gap-4">
                        ...
                        <p className="text-sm sm:text-gray-700 pt-1 sm:pt-2 leading-relaxed">{instruction}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {recipe.tips && (
                  <div className="bg-gradient-to-r from-filipino-light-gold to-filipino-cream rounded-xl p-4 sm:p-6 border-l-4 border-filipino-gold">
                    <div className="flex items-start gap-2 sm:gap-3">
                      ...
                      <div>
                        <h3 className="text-sm sm:text-lg font-bold text-gray-800 mb-1 sm:mb-2">Cooking Tips</h3>
                        <p className="text-sm sm:text-gray-700">{recipe.tips}</p>
                      </div>
                    </div>
                  </div>
                )}
